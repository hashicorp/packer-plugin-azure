...
- `skip_create_build_key_vault` (bool) - Set to true to skip creating the build key vault during Windows build.
  This is useful for cases when subscription has policy restrictions on key vault resource. If this flag is set to true, customer has to provide alternate method to setup WinRM during Windows build, else build will fail. A few examples for this can be found in the windows_skip_key_vault directory of the plugin example directory.  The commands to setup WinRM with certificate is specified in user_data_file file which then directly gets executed on Packer VM using custom_script.
  These examples provides a minimal setup needed to get a working solution with `skip_create_build_key_vault` flag.  This is a complex topic on Windows and may require changes to the script depending on the version of Windows used, or if any changes are made to the existing versions of Windows that impact creation of a WinRM listener. See [documentation](https://docs.microsoft.com/en-us/azure/virtual-machines/extensions/custom-script-windows) to learn more about custom_script. See [documentation](https://learn.microsoft.com/en-us/azure/virtual-machines/user-data) to learn more about user data.
