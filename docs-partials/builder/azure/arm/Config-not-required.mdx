...
- `build_disk_encryption_set_id` (string) - Specify the Disk Encryption Set ID to use to encrypt the OS and data disks created with the VM during the build
...
- `custom_script` (string) - Specify a command to inject into the CustomScriptExtension, to run on startup
