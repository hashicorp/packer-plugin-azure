...
- `allowed_inbound_ip_addresses` ([]string) - Specify the list of IP addresses, CIDR blocks, and hostnames that should be
  allowed access to the VM. Hostnames are resolved to literal IP addresses at build time and all resolved addresses are used.
  If provided, a temporary Azure Network Security Group is created and attached to the subnet for builder-managed VNets or to the NIC for existing VNets.
  Builder-managed VNet behavior is unchanged for backward compatibility. The temporary NSG is removed during cleanup.

- `deny_outbound_ip_addresses` ([]string) - Specify the list of IP addresses, CIDR blocks, and hostnames that the temporary
  build VM must not reach over outbound traffic during image creation. Hostnames are resolved to literal IP addresses at build time and the deny list is a build-time snapshot.
  This applies only to the temporary build VM and does not change the networking of VMs created later from the image.
